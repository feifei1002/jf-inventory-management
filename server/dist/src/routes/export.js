...
router.get("/pr/:requisitionId", async (req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename=${pr.requisitionId}.xlsx`);
...
router.get("/po/:purchaseId", async (req, res) => {
    try {
        ...
        // ── Stream response ──
        res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.setHeader("Content-Disposition", `attachment; filename=${po.purchaseId}.xlsx`);
        await workbook.xlsx.write(res);
        res.end();
    }
    catch (error) {
